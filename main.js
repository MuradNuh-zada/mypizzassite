...
const pizzasDOM = document.querySelector(".pizzas"),
    inputDOM = document.getElementById("input_pizza"),
    notfoundimageDOM = document.querySelector(".notfound"),
    cartDOM = document.querySelector(".list-pizza"),
    favDOM = document.querySelector(".fav-pizza"),
    cartBTNElem = document.querySelector(".cart-btn"),
    deleteCartBTN = document.querySelector("#delete-card"),
    favListBTN = document.querySelector(".fav-list-btn"),
    favListELEM = document.querySelector(".fav-list"),
    favCountELEM = document.querySelector(".countFav span"),
    deleteFavListELEM = document.querySelector("#delete-favlist");
let listElem = document.querySelector(".list"),
    cartPriceELem = document.querySelector("#cartPrice span"),
    countBasketBtn = document.querySelector(".countElem span"),
    allPrice = document.querySelector(".all-price span"),
    cartList = [],
    favList = [];


// TODO: Cart list on/off
let clickedCartBTN = false;
cartBTNElem.addEventListener("click", () => {
    clickedCartBTN = !clickedCartBTN
    if (clickedCartBTN) {
...
// TODO: Fav list on/off
let clickedFavListBTN = false;
favListBTN.addEventListener("click", () => {
    clickedFavListBTN = !clickedFavListBTN;
    if (clickedFavListBTN) {
        favListELEM.style.display = "block";
    } else {
        favListELEM.style.display = "none";
    }
})

// TODO: Product Render
function render(renderedList) {
    pizzasDOM.innerHTML = ""
    renderedList.forEach(item => {
        ...
        <div class="pizza">
            <img src="${item.img}"
            alt="${item.name}"
            onerror="this.src='./images/notfound-image.jpg'"/>
            <div class="pizza_info">
                <h2 class="pizza_name">${item.name}</h2>
                <p class="pizza_desc">${item.dsc}</p>
                <span class="pizza_price">${item.price} UAH</span>
                <button onclick="addToCart('${item.id}')" onmouseenter="changeText(this)" onmouseleave="normalText(this)" id="addBtn"><i class="fa-solid fa-plus"></i></button>
            </div>
            <span class="non-liked" onclick="toggleLiked('${item.id}')">
            ${item.liked ? '<i class="fa-solid fa-heart liked"></i>' : '<i class="fa-regular fa-heart"></i>'}
        </span>
...
function renderList() {
    ...
    cartList.forEach(item => {
        ...
        <div class="pizza-info">
            <img src="${item.img}"
            alt="">
            <div class="pizza-info-desc">
                <h3>${item.name}</h3>
                <p>${item.dsc}</p>
                <p class="item-price">${item.price * item.count}<sup>$</sup></p>
                <button onclick="decraese('${item.id}')">➖</button>
                <span>${item.count}</span>
                <button onclick="increase('${item.id}')">➕</button>
                <div class="delete-item-div">
                    <button class="delete-item" onclick="deleteListItem('${item.id}')"><span>❌</span></button>
                    <p class="delete-text">🥺</p>
                </div>
              </div>
        </div>`
    })
    let totalPrice = cartList.reduce((acc, item) => acc += item.price * item.count, 0)
    allPrice.textContent = totalPrice;
    cartPriceELem.textContent = `${totalPrice !== 0 ? `${totalPrice}$` : "Cart is empty!"}`;
}

// TODO: Fav list render
    ...
    favList.forEach(item => {
        ...
        <div class="pizza-info">
            ...
            <div class="pizza-info-desc">
                <h3>${item.name}</h3>
                <p>${item.dsc}</p>
                <p class="item-price">${item.price}<sup>$</sup></p>
            </div>
            <p class="deleteFavPizza" onclick="deleteFavPizza('${item.id}')"><i class="fa-solid fa-xmark"></i></p>
        </div>`
    })
}

// TODO: Liked or non-liked button 
products.forEach(item => item.liked = false);
function toggleLiked(id) {
    let favListCount = 0;
    let selectedItem = products.find(item => item.id === id);
    selectedItem.liked = !selectedItem.liked; // Toggle the liked state for the selected item
    if (selectedItem.liked) {
        let findFavPizza = { ...selectedItem };
        favList.push(findFavPizza);
        favList.forEach(item => item.favCount = 1);
        renderFavList();
    } else {
        let filterFavList = favList.filter(item => item.id !== id);
        favList = filterFavList;
        renderFavList();
    }

    favList.forEach(item => favListCount += item.favCount)
    favCountELEM.textContent = favListCount
    render(products);// Re-render products after toggling
}

// TODO: Remove the pizza from the Fav list  
function deleteFavPizza(id) {
    let findDeleteFavPizza = products.find(item => item.id === id);
    findDeleteFavPizza.liked = !findDeleteFavPizza.liked;
    let filterFavList = favList.filter(item => item.id !== id);
    favList = filterFavList;
    let favListCount = 0;
    favList.forEach(item => favListCount += item.favCount)
...
// TODO: Deleted Fav List
deleteFavListELEM.addEventListener("click", () => {
    products.forEach(item => item.liked = false);
    favList = [];
...
function deleteListItem(id) {
    const filteredListItem = cartList.filter(prod => prod.id !== id);
    let deleteItemCount = 0
    filteredListItem.forEach(item => deleteItemCount += item.count);
    countBasketBtn.innerText = deleteItemCount
    cartList = filteredListItem;
    renderList()
}
...
deleteCartBTN.addEventListener("click", () => {
    cartList = []
    let countBasket = 0;
    countBasketBtn.textContent = countBasket;
    cartPriceELem.textContent = "Cart is empty!"
    renderList()
})

// TODO: Increase the number of products
function increase(id) {
    let countBasket = 0;
    ++cartList.find(item => item.id === id).count;
    cartList.forEach(item => countBasket += item.count);
    countBasketBtn.textContent = countBasket
    renderList()
}

// TODO: Decraese the number of products
function decraese(id) {
    let countBasket = 0;
    let decreaseCount = --cartList.find(item => item.id === id).count;
    if (decreaseCount === 0) {
        const filteredProducts = cartList.filter((prod) => prod.id !== id)
        cartList = filteredProducts
    };
    cartList.forEach(item => countBasket += item.count);
    countBasketBtn.textContent = countBasket
    renderList()
...
function normalText(param) {
    param.innerHTML = `<i class="fa-solid fa-plus"></i>`;
}
function changeText(param) {
    param.innerText = "Take me 🙃";
}

// TODO: Searching
inputDOM.addEventListener("keyup", (e) => {
    let filteredDATA = products.filter(item => {
        let nameData = item.name.toLowerCase()
        let descData = item.dsc.toLowerCase()
        let inputData = e.target.value.toLowerCase().trim()
        let searchingData = nameData.includes(inputData) || descData.includes(inputData)
        return searchingData;
    })
    if (filteredDATA.length) {
        render(filteredDATA);
        notfoundimageDOM.style.display = "none";
        cartDOM.style.display = "block";
    } else {
        pizzasDOM.innerHTML = ""
        notfoundimageDOM.style.display = "block";
